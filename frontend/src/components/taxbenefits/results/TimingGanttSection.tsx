...
interface TimingGanttSectionProps {
  ...
  // Hardcoded to July (7) until this component is replaced by date-driven gantt (IMPL-115)
  // exitMonth removed (IMPL-117) — now engine-internal, auto-derived from timeline
  // Hardcoded to December (12) until this component is replaced by date-driven gantt (IMPL-115)
  ozEnabled: boolean;
}
...
const TimingGanttSection: React.FC<TimingGanttSectionProps> = ({
  ...
  // IMPL-117: placedInServiceMonth is now engine-internal. Hardcode to July until IMPL-115 replaces this component.
  const placedInServiceMonth = 7;
  // IMPL-117: exitMonth is now engine-internal. Hardcode to December until IMPL-115 replaces this component.
  const exitMonth = 12;
  ...
  const { holdFromPIS, totalInvestmentYears, exitYear, delaySpilloverYears } = useMemo(() => {
    ...
    const totalMonths = constructionDelayMonths + (creditPeriodFromPIS * 12);
    const total = Math.ceil(totalMonths / 12) + 1;
    const exit = prePIS + creditPeriodFromPIS + 1;
    return { holdFromPIS: creditPeriodFromPIS, totalInvestmentYears: total, exitYear: exit, delaySpilloverYears: total - exit };
  }, [constructionDelayMonths, creditPeriodFromPIS]);
  ...
  }, [constructionDelayMonths, placedInServiceMonth, exitMonth, exitYear, ozEnabled, creditPeriodFromPIS]);
  ...
  return (
    <CollapsibleSection title={`Timing Clock Gantt Chart — ${totalInvestmentYears} year timeline`} defaultExpanded={false}>
      ...
      <div className="mt-2 px-1 text-xs" style={{ color: '#9ca3af' }}>
        ...
        {constructionDelayMonths} + ({creditPeriodFromPIS} x 12) ={' '}
        {constructionDelayMonths + creditPeriodFromPIS * 12} months{' '}
        = ceil({constructionDelayMonths + creditPeriodFromPIS * 12}/12) + 1 dispo{' '}
