...
export const GanttHeader: FC<GanttHeaderProps> = ({ className }) => {
  ...
  return (
    <div
      className={cn(
        "-space-x-px flex h-full w-max divide-x divide-border/20",
        className
      )}
    >
      <Header />
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center gap-2.5 p-2.5 text-xs hover:bg-white/30",
        className
      )}
      key={feature.id}
      onClick={handleClick}
    ...
    >
      ...
      />
      <p className="pointer-events-none flex-1 truncate text-left font-medium min-w-0" title={feature.name}>
        {feature.name}
      </p>
      <p className="pointer-events-none text-muted-foreground shrink-0 hidden sm:block">{duration}</p>
    </div>
  );
};
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-b border-gray-300/50 p-2.5 font-medium text-xs"
    style={{ height: "var(--gantt-header-height)", color: '#1C3333' }}
  >
    <p className="flex-1 truncate text-left">Timing Clocks</p>
    <p className="shrink-0 text-muted-foreground">Duration</p>
  </div>
);
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    ...
    <div className="divide-y divide-gray-300/50">{children}</div>
  </div>
);
...
export const GanttSidebar: FC<GanttSidebarProps> = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const startWidth = sidebarRef.current?.getBoundingClientRect().width ?? 300;
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      if (sidebarRef.current) {
        ...
        // Update CSS variable for timeline offset
        const ganttEl = sidebarRef.current.closest('[data-roadmap-ui="gantt"]');
        if (ganttEl) {
          (ganttEl as HTMLElement).style.setProperty('--gantt-sidebar-width', `${newWidth}px`);
        }
      }
    };
  ...
  return (
    <div
      ref={sidebarRef}
      className={cn(
        "sticky left-0 z-30 min-h-full overflow-hidden border-r border-gray-300/60 relative",
        className
      )}
      ...
      style={{
        ...
        minWidth: 140,
        maxWidth: 600,
        backgroundColor: 'rgba(237, 245, 245, 0.78)',
    ...
    >
      ...
      <div
        className={cn(
          "absolute top-0 right-0 w-1.5 h-full cursor-col-resize z-40 group hover:bg-primary/20 transition-colors",
          isResizing && "bg-primary/30"
        )}
        onMouseDown={handleMouseDown}
      >
        <div className={cn(
          "absolute top-1/2 -translate-y-1/2 right-0 w-0.5 h-8 rounded-full bg-muted-foreground/30 transition-opacity",
...
export const GanttColumns: FC<GanttColumnsProps> = ({
  ...
  return (
    <div
      className="divide grid h-full w-full divide-x divide-border/20"
      style={{
        gridTemplateColumns: `repeat(${columns}, var(--gantt-column-width))`,
      }}
    >
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <div
      data-slot="gantt-card"
      className="h-full w-full rounded-md p-2 text-xs"
      style={statusColor ? {
        backgroundColor: statusColor,
        color: '#fff',
        border: '1px solid rgba(255,255,255,0.2)',
        boxShadow: `0 2px 8px ${hexToRgba(statusColor, 0.4)}`,
      } : {
...
export type GanttProviderProps = {
  ...
  children: ReactNode;
  className?: string;
  /** Override timeline start year (default: current year - 1) */
  startYear?: number;
  /** Override timeline end year (default: current year + 1) */
  endYear?: number;
};
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const [timelineData, setTimelineData] = useState<TimelineData>(() => {
    if (startYear && endYear) {
      // Generate fixed timeline covering the specified range
      const data: TimelineData = [];
      for (let y = startYear; y <= endYear; y++) {
        data.push({
          year: y,
          quarters: new Array(4).fill(null).map((_, qi) => ({
            months: new Array(3).fill(null).map((_, mi) => ({
              days: getDaysInMonth(new Date(y, qi * 3 + mi, 1)),
            })),
          })),
        });
      }
  ...
  // Start scrolled to the left (beginning of timeline), not centered
  useEffect(() => {
  ...
  // Update sidebar width when DOM is ready — reads actual width for resize support
  useEffect(() => {
    const updateSidebarWidth = () => {
      const sidebarElement = scrollRef.current?.querySelector(
        '[data-roadmap-ui="gantt-sidebar"]'
      ) as HTMLElement | null;
      const newWidth = sidebarElement ? sidebarElement.getBoundingClientRect().width : 0;
  ...
  const handleScroll = useCallback(
    throttle(() => {
      ...
      // Infinite scroll disabled — fixed investment timeline does not extend dynamically.
      // Users scroll within the pre-computed timeline range only.
    }, 100),
    []
  );
