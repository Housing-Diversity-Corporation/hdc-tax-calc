...
describe('HDC Waterfall Comprehensive Tests', () => {
  ...
  const baseParams: CalculationParams = {
    ...
    investorUpfrontCash: 2000000, // 20% of 10M
  ...
  describe('2. AUM Fee Tests', () => {
    ...
    test('AUM Fee with 50% Current Pay / 50% PIK', () => {
      ...
      console.log('Test 2c - AUM Fee 50/50 Split:', {
        paid: year1.aumFeePaid ?? 0,
        accrued: year1.aumFeeAccrued ?? 0,
  ...
  describe('3. Outside Investor Sub-Debt Tests', () => {
    ...
    test('Outside Investor PIK Accumulation', () => {
      ...
      const results = calculateFullInvestorAnalysis(params);
      const year1 = results.investorCashFlows[0];
      const year10 = results.investorCashFlows[9];

      expect(year1.outsideInvestorCurrentPay).toBe(0);
      expect(year1.outsideInvestorPIKAccrued).toBeCloseTo(120000, 0);

      // Check that final exit value reflects the accumulated debt
      expect(results.outsideInvestorSubDebtAtExit).toBeGreaterThan(1000000);
      ...
      console.log('Test 3b - Outside Investor PIK:', {
        ...
        totalGrowth: ((results.outsideInvestorSubDebtAtExit / 1000000 - 1) * 100).toFixed(1) + '%'
      });
    });
  });
  ...
  describe('4. DSCR Cash Management System', () => {
    test('Should maintain 1.05x DSCR with multiple obligations', () => {
      ...
      console.log('Test 4 - DSCR Management:', {
        ...
        hasDeferrals,
        year1Details: {
          dscr: results.investorCashFlows[0].dscr ?? 0,
          aumPaid: results.investorCashFlows[0].aumFeePaid ?? 0,
          aumDeferred: results.investorCashFlows[0].aumFeeAccrued ?? 0
        }
      });
    });
  ...
  describe('5. Complex Interaction Tests', () => {
    test('All features enabled simultaneously', () => {
      ...
      const results = calculateFullInvestorAnalysis(params);
      const hdcResults = calculateHDCAnalysis(params as any);
      ...
      // Check HDC analysis
      expect(hdcResults.totalHDCReturns).toBeGreaterThan(0);
      // Note: hdcMultiple is 0 because HDC has $0 initial investment (cannot calculate multiple)
      expect(hdcResults.hdcMultiple).toBe(0);

      // Check OZ impact (Year 5 tax payment)
      ...
      console.log('Test 5 - All Features Enabled:', {
        ...
        hdcIRR: hdcResults.hdcIRR.toFixed(2) + '%',
        hdcMultiple: hdcResults.hdcMultiple.toFixed(2) + 'x',
        year1Components: {
          noi: year1.noi,
        ...
        year5OZTax: year5.ozYear5TaxPayment
      });
    });
    ...
    test('Exit proceeds waterfall distribution', () => {
      ...
      const exitValue = results.exitProceeds || 0;
      ...
      console.log('Test 5b - Exit Waterfall:', {
        ...
        remainingDebtAtExit: results.remainingDebtAtExit || 0,
        outsideInvestorFinal: results.outsideInvestorSubDebtAtExit || 0,
