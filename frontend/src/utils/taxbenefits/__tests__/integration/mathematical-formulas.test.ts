...
describe('Every Mathematical Formula - Explicit Tests', () => {
  ...
  describe('Capital Structure Formulas', () => {
    ...
    it('Total Investment = Investor Equity (NOT including HDC Fee)', () => {
      const investorEquity = 10;
      const totalInvestment = investorEquity; // HDC fee is NOT part of investor's investment
      expect(totalInvestment).toBe(10);
    });
  });
  ...
  describe('Tax Benefit Formulas', () => {
    ...
    it('HDC Fee = Tax Benefit × 10%', () => {
      ...
      expect(hdcFee).toBeCloseTo(0.5383125, 6); // Increased precision to 6 decimals
    });
  ...
  describe('Debt Service Formulas', () => {
    ...
    it('PMT = P × (r(1+r)^n) / ((1+r)^n - 1)', () => {
      ...
      expect(monthlyPayment).toBeCloseTo(0.179865, 5); // Increased precision to 5 decimals
    });
  ...
  describe('PIK Interest Formulas', () => {
    ...
    it('PIK Portion = Balance × Rate × (1 - Current Pay %)', () => {
      ...
      expect(pikPortion).toBeCloseTo(0.54, 6); // Increased precision to handle floating point
    });
  });
  ...
  describe('Soft Pay Waterfall Formulas', () => {
    ...
    it('PIK Accrual = Interest Due - Amount Paid', () => {
      ...
      expect(pikAccrual).toBeCloseTo(0.6, 6); // Increased precision to handle floating point
    });
    ...
    it('PIK Balance Compounds: New Balance = Old Balance + Accrued Interest', () => {
      ...
      expect(pikAccrual).toBeCloseTo(0.6, 6); // Increased precision to handle floating point
      expect(newBalance).toBeCloseTo(10.6, 6); // Increased precision to handle floating point
    });
  });
  ...
  describe('Exit Calculation Formulas', () => {
    ...
    it('Investor Share = Gross × Investor Promote %', () => {
      const grossProceeds = 40;
      const investorPromote = 0.65;
      const investorShare = grossProceeds * investorPromote;
      expect(investorShare).toBe(26);
    });

    it('HDC Share = Gross × (1 - Investor Promote %)', () => {
      const grossProceeds = 40;
      const investorPromote = 0.65;
      const hdcShare = grossProceeds * (1 - investorPromote);
      expect(hdcShare).toBe(14);
    });
