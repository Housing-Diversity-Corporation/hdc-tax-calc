...
| IMPL-165 | Cash sweep waterfall — phil sweep first then DDF. New philSweepPct and hdcDebtFundSweepPct params take percentage of CADS surplus after DSCR + current-pay + AUM catch-up to reduce PIK balances year-over-year. Phil sweep runs before DDF (priority order). UI sliders in CapitalStructureSection. Fixes defects #7 (no AMZN/phil sweep) and #9 (DDF never paid down from surplus) from Phase 2 Trace 4001 audit. | Deployed | 2026-05-02 | calculations.ts, index.ts, useHDCState.ts, useHDCCalculations.ts, HDCCalculatorMain.tsx, HDCInputsComponent.tsx, CapitalStructureSection.tsx, exportParamsSync.test.ts, impl-165-cash-sweep.test.ts | 1,906 tests (101 suites, 0 failures). |
| IMPL-166 | Developer Deferred Fee (C Note) — devFeeTotal with closing piece (reduces net equity at Year 0) and deferred balance paid from CADS surplus at Priority 5 (after phil/DDF sweeps). Face value, zero interest, no PIK compounding. Remaining balance deducted at exit before LP profit split. UI inputs in CapitalStructureSection. | Deployed | 2026-05-02 | calculations.ts, calculationGuards.ts, index.ts, useHDCState.ts, useHDCCalculations.ts, HDCCalculatorMain.tsx, HDCInputsComponent.tsx, CapitalStructureSection.tsx, exportParamsSync.test.ts, impl-166-dev-fee.test.ts | 1,912 tests (102 suites, 0 failures). |
| IMPL-167+ | *Unassigned -- available for future work* | -- |
...
### IMPL-195: OZ Returns Buildup — Symmetric Avoided-Tax Memos (display-only)

**Status:** ✅ Complete (2026-07-20). Display-only; no engine files touched. MOIC and IRR unchanged.

**IMPL-number note:** the driving prompt assumed this was "IMPL-194," but IMPL-194 is already claimed for the HFA Knowledge Base frontend work ("HFA lookup and pre-population in deal creation flow" — `HDC_Platform_Build_Sequence_v1_0.md`, `HDC_HFA_Knowledge_Base_Spec_v1_1.md`, `HDC_Canonical_Schema_Spec_v1_2.md`). Claimed the next free number, **IMPL-195** (189–194 taken by composable-sources/HFA work; 200–213 by Wave 1). Flagging the collision so IMPL-194 stays with HFA.

**Context (verification bundle, Parts 1 & 3 were read-only):** verified on preset `701 BG 60.5M 3.5M Sponsor First Loss` (WA Non-REP), MOIC 3.54x / IRR 14.12% / Total Returns $50.40M.
- **Part 1 (gate, read-only):** for OZ 10-year holds the engine reflects both avoided taxes ONCE via the tax-free exit line (`netExitTax=0`, `calculations.ts:212`) and the IMPL-134 extended path **excludes** `ozRecaptureAvoided` and `ozExitAppreciation` from the reported `totalReturns` and IRR (`calculations.ts:2532-2544`; displayed IRR = `results.irr` = `adjustedIRR`, `KPIStrip.tsx:313`). Gate **passed**. ISS-076's "recapture inflates the IRR" is a **misattribution** — recapture is in the base-path annual returns *table* (`calculations.ts:1861`, `:1928`) but NOT in the reported IRR/MOIC (IMPL-134 already strips it). Capital-gains-on-appreciation avoided ≈ **$0** on this preset (exit proceeds ≈ cumulative depreciation → residual appreciation floors at 0).
- **Part 3 (read-only, reported separately):** "Excess Reserve" ($1.20M) returns the full, undrawn, largely debt-funded interest reserve as a Year-2 distribution (`interestReservePeriodYears=ceil(16/12)=2`, `calculations.ts:1751`) BEFORE the draw window opens at PIS Year 3 (`:1131`; Year-3 DSCR 0.43x is the uncovered shortfall). Overstates MOIC by up to the displayed 0.08x and inflates IRR via early timing. Confirms/elevates **ISS-077**. No fix applied.

**Change (Part 2, per Brad's decision — restructure parent to additive-only):**
- `ReturnsBuiltupStrip.tsx`: the OZ Benefits parent now shows only its **additive** contribution — `ozDeferralNPV + ozStepUpSavings` (~$1.42M / 2.8% on the 701 preset), **summed from the engine's own child values** (no display-side `9.72 − 8.30` subtraction, so it cannot drift).
- **Recapture Avoided** ($8.30M) and **Capital Gains Avoided** (`ozExitAppreciation`, ~$0 here) are rendered below the line as **non-additive memos** — always rendered (including at ~$0, shown as "~$0"), marked "(memo · non-additive)", with symmetric counterfactual framing ("the [recapture / capital-gains] tax a non-OZ investor would owe on this position — eliminated by the 10-year step-up; reflected in Exit Proceeds (net), not added again"). Multiple and percent-of-total columns are blanked ("—") for memo rows.
- Result: the five additive components (Depreciation 22.4% + OZ 2.8% + Operating Cash 10.2% + Excess Reserve 2.4% + Exit Proceeds 62.2%) now sum to **$50.40M / 100.0%** (previously ~116.5% because the OZ parent double-showed the $8.30M recapture memo additively).
