...
| ISS-076 | **OZ recapture-avoided recognized in Year 3 instead of at exit.** `ozRecaptureAvoided` (calculations.ts:1827-1832) is booked annually as depreciation is taken; bonus depreciation front-loads the benefit into Year 3 (~10 yrs before the sale that creates it), inflating early IRR. Confirmed timeline: 2-yr construction → PIS Year 3 → LIHTC starts Year 4, so there is NO LIHTC in Year 3 and nearly all of Year 3's ~$6.96M non-depreciation amount is the mistimed OZ benefit (larger than first logged). Benefit is contingent on 10-yr hold + sale. Bonus dep tax benefit itself is correctly in Year 3 — only recapture-avoided is mistimed. Related guards at L2281/L2540 must be reconciled by any fix. Full writeup + proposed resolution: `audits/OZ-RECAPTURE-AVOIDED-TIMING-ISS-076-2026-07-17.md`. Discovered on `701 BG 64M TDC` (IRR 10.1%). | High | Logged 2026-07-17 — OPEN, no fix applied |
| ISS-077 | **Interest reserve full balance distributed in Year 2 (needs verification).** Engine books `excessReserveDistribution = interestReserveBalance` (calculations.ts:1750-1758) — the whole ~$1.128M — in Year 2, likely still mid-lease-up. Reserve is capitalized into TPC to fund the post-construction lease-up shortfall, so only a genuine *excess* after the S-curve draw should be distributed, and at the right time. Confirm the draw consumes lease-up need before distribution. Detail: "Related Open Question" in `audits/OZ-RECAPTURE-AVOIDED-TIMING-ISS-076-2026-07-17.md`. Discovered on `701 BG 64M TDC`. | Low | Logged 2026-07-17 — OPEN, unverified |
| ISS-078 | ~~Save-as silently overwrites an existing config~~ **CLOSED** — The save dialog prompted for a new name but `handleSaveConfiguration` decided PUT-vs-POST solely on whether a config was loaded (HDCInputsComponent.tsx:864), ignoring the entered name, so saving a loaded config under a new name renamed/clobbered the original (caused loss of `701 BG 64M TDC`). Fix: PUT only when the entered name matches the loaded config's name, else POST a new record; SaveConfiguration.tsx shows a dynamic Save as New / Update button + an inline overwrite confirmation. The confirmation was first built as a nested Radix AlertDialog inside the save Dialog, which leaked `pointer-events:none` and froze the app; reworked to an inline confirmation in the same Dialog. Runtime-verified: new name → new record; matching name → confirm. | High | Fixed 2026-07-18 |
